...
for imovie in range(n_movie):
    print('start processing {}-{}'.format(imovie + 1, str(movie_list[imovie])[2:-6]))
    ...
    # tif_rep = np.concatenate(np.split(tif_imovie_with_mean, tif_imovie_with_mean.shape[3], axis=3),
    #                          axis=1).squeeze().transpose(2, 0, 1)
    tif_imovie_mean = tif_imovie_mean.transpose(2, 0, 1)
    imwrite(pjoin(path_out_tif, str(movie_list[imovie])[2:-6] + '.tif'), tif_imovie_mean, imagej=True)
print('export all mean-tifs')
np.save(pjoin(path_out, 'snr.npy'), snr)

# %% plot snr
snr = np.load(pjoin(path_out, 'snr.npy'))
plot_movie_snr(snr, 4, 4, movie_list, patches=None, vmin=0.2, vmax=4, pixel_um=13,
               path_out=path_out)
...
for imovie in range(n_movie):
    movie_name = str(movie_list[imovie])[2:-6]
...
patch_pca_var_df = pd.DataFrame(patch_pca_var, index=patch_list, columns=['PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'PC8', 'PC9', 'PC10'])
...
[n_cluster_optimal, aic_all, bic_all, silhouette_score_all, labels_arr_all, _] = optimal_cluster_gmm(u_pca,
    n_cluster_max, random_init=True, n_init=1000)
