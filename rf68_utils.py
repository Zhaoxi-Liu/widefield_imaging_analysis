...
#%%
def amp_plot(amp, cmap='hsv', title=None, path_out=None, pixel_um=None, range=None):
    ...
    if path_out is not None:
        plt.savefig(pjoin(path_out, title + '.png'), bbox_inches='tight')
    plt.show()

#%%
def plot_response_RF(data, window=[0, 20], title=None, path_out=None):
    n_rows = 6
    n_cols = 8
    ylim_max = data.max()
    ylim_min = data.min()
