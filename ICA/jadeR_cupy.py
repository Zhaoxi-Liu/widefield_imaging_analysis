...
import os
import getopt
from cupy import abs, append, arange, arctan2, argsort, array, concatenate, \
    cos, diag, dot, eye, float64, float32, loadtxt,  multiply, ndarray, \
    savetxt, sign, sin, sqrt, zeros, save
from cupy.linalg import eigh, pinv

__version__ = 1.8


def jadeR_cupy(X, m=None, verbose=False):
    """
    ...
    #     eigensum = eigensum + Ds[i]
    #     countEigen += 1
    #     if eigensum > (sum(Ds)*0.95):
    #         m=countEigen
    #         print(m)
    #         break
    ...
    CM = zeros([m,m*nbcm], dtype=float64)
    R = eye(m, dtype=float64)
    # Temp for a cum. matrix
    Qij = zeros([m,m], dtype=float64)
    # Temp
    ...
    for im in range(m):
        ...
        first = zeros(X.shape)
        for i in range(X.shape[1]):
            first[:,i] = multiply(Xijm,X[:,i])

        # Qij = dot(multiply(Xijm, X).T ,X )/ float(T) - R - 2 * (dot(R[:,im] , R[:,im].T))
        
        Qij = dot(first.T ,X )/ float(T) - R - 2 * (dot(R[:,im].reshape((-1,1)) , R[:,im].reshape((-1,1)).T))

        CM[:,Range] = Qij 
        Range = Range  + m 
        for jm in range(im):
            Xijm = multiply(Xim, X[:,jm])
            first1 = zeros(X.shape)
            for i in range(X.shape[1]):
                first1[:,i] = multiply(Xijm,X[:,i])
            Qij = sqrt(2) * (dot(first1.T , X) / float(T) 
                - dot(R[:,im].reshape((-1,1)) , R[:,jm].reshape((-1,1)).T) - dot(R[:,jm].reshape((-1,1)) , R[:,im].reshape((-1,1)).T))
            # if jm ==0:
            #       a= Qij
            #       save('./check_cupy/c1.npy',a)
            CM[:,Range]	= Qij
            Range = Range + m
    del first ,first1
    # save('./check_cupy/c2.npy',CM)
    ...
    for im in range(nbcm):
        Diag = diag(CM[:,Range])
        On = On + (multiply(Diag,Diag)).sum(axis=0)
        Range = Range + m
    second = (multiply(CM,CM).sum(axis=0)).reshape(1,-1)
    Off = second.sum(axis=1) - On
    ...
    while encore:
        ...
        for p in range(m-1):
            for q in range(p+1, m):
                ...
                third = (CM[p,Ip].reshape(1,-1)) - (CM[q,Iq].reshape(1,-1))
                fourth = (CM[p,Iq].reshape(1,-1)) + (CM[q,Ip].reshape(1,-1))
                g = concatenate([third, fourth])
    ...
    A = pinv(B)
    fifth = multiply(A,A).sum(axis=0).reshape(1,-1)
    sixth = array(argsort(fifth[0])).reshape(1,-1)
    # print(sixth.shape)
    keys =  sixth[0]
    # print(multiply(A,A).shape)
    # print(multiply(A,A).sum(axis=0).shape)
    # print(multiply(A,A).sum(axis=0)[0].shape)
    # print(keys.shape)
    B = B[keys,:]
    # % Is this smart ?
    B = B[::-1,:]
    ...
    seventh = array(sign(sign(b)+0.1).T).reshape(1,-1)
    signs = seventh[0]
    B = dot(diag(signs) , B)
