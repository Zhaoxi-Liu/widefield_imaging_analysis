...
def calculate_dff(data: np.ndarray) -> np.ndarray:
    '''
    data: 2D numpy.ndarray, the first dimension is temporal samples
    '''
    data_baseline = np.percentile(data, q=10, axis=0)
    dff = (data - data_baseline) / data_baseline
    return dff

def calculate_dff_norm(x1: np.ndarray, x2: np.ndarray) -> np.ndarray:
    """
    Calculate the delta F/F between two signals.
    x1, x2: 2D array, each row is a signal.
    """

    x1_norm = x1 / x1.max(axis=0)
    print(x1_norm.shape)
    x2_norm = x2 / x2.max(axis=0)
    print(x2_norm.shape)
    dff = (x1_norm - x2_norm) / x2_norm
    return dff

def contours_to_pixels(contours, shape):
    '''
    Convert the contours to mask.
    contours: list of np.ndarray, each element is a contour of a roi,
    pixels: np.ndarray, shape is (2, n), n is the number of pixels in the mask,
    the first row is the x coordinates and the second row is the y coordinates.
    
    '''
    mask = np.zeros(shape, dtype=np.uint8)
    for contour in contours:
        cv2.fillPoly(mask, pts=[contour], color=1)
    ypix, xpix = np.where(mask == 1)
    pixels = np.array([xpix, ypix])
    return pixels
...
def rois_to_contours(rois_path):
    '''
    Read the rois from the ImageJ rois .zip file and convert them to contours.
    contours: list of np.ndarray, each element is a contour of a roi,
    in each contour, the first column is the x coordinates and the second column
    ...
    roi_contours = []
    for roi in rois:
        if roi.roitype == 7: # ROI_TYPE.FREEHAND
            _rois = np.zeros_like(roi.integer_coordinates)
            _rois[:,0] = roi.integer_coordinates[:,0] + roi.left
            _rois[:,1] = roi.integer_coordinates[:,1] + roi.top
        roi_contours.append(_rois)
    print('The number of rois:', len(rois))
    return roi_contours

def rois_to_pixels(rois_path, shape):
    '''
    Read the rois from the ImageJ rois .zip and convert them to mask pixels.
    pixels: np.ndarray, shape is (2, n), n is the number of pixels in the mask,
    the first row is the x coordinates and the second row is the y coordinates.
    
    '''
    contours = rois_to_contours(rois_path)
    pixels = contours_to_pixels(contours, shape)
    return pixels
